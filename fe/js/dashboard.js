...
socket.on("sensor_update", (data) => {
  ...
  if (data.light > 1000) {
    ...
    lightIcon.classList.add("bi-brightness-high", "text-warning", "glow-effect");
  }
...
document.querySelectorAll('.device-switch').forEach(switchEl => {
  switchEl.addEventListener('change', async function (e) {
    ...
    try {
      const res = await fetch(`http://localhost:3000/devices/${deviceId}/control`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: targetState })
      });
